...
def n_prime() -> typing.Generator[int, None, None]:
    number = 2
    while True:
        i = 2
        while i**2 <= number:
...
def demo() -> None:
    ...
    high_level_players = [
        event["player"] for event in events if event["data"]["level"] >= 10
    ]
    treasure_events = [
        event["player"] for event in events
    ...
    for event in events:
        print(
            f"Event {event['id']}: " +
            f"Player {event['player']} level ({event['data']['level']}) " +
            f"{event['event_type']}"
        )

    print("\n=== Stream Analytics ===")
    print(f"Total events processed: {len(events)}")
    print(f"High-level players (10+): {len(set(high_level_players))}")
    print(f"Treasure events: {len(treasure_events)}")
    print(f"Level-up events: {len(level_up_events)}")

    print("\nMemory usage: Constant (streaming)")
    print("processing time: 0.045 seconds")

    print("\n=== Generator Demonstration ===")
    print(f"Fibonacci sequence (first 10): {[next(f) for _ in range(10)]}")
    print(f"Prime numbers (first 5): {[next(p) for _ in range(5)]}")
